...
A local-first web-app starter, optimized for live iPad-over-LAN iteration. Static-only deploy to GitHub Pages, all state in IndexedDB, and a single `bun run check` that gates every change.
...
| State | Jotai | `2.x` (parameterized atoms via the `atomWithIDB` key + a module-scope `Map<id, atom>`; `selectAtom` from `jotai/utils` for derived per-id slices — house-manager does not use `atomFamily`) |
...
```bash
git clone https://github.com/<owner>/house-manager.git
cd house-manager
...
Open `apps/web/.env` and confirm `VITE_GAME_TITLE` is set (the committed default `house-manager` is fine for development).

> The text lockfile **`bun.lock`** is committed. The older binary form `bun.lockb` is gitignored. Don't commit it.

> Workspace-internal deps use the `workspace:*` protocol — never a published version range. Bun creates symlinks under `node_modules/@house-manager/*` so Biome, Stylelint, and TS can extend the shared configs.
...
**Fix the cause, not the symptom.** Manual `useMemo` / `useCallback` / `React.memo` is forbidden in house-manager (the React Compiler handles memoization — see `.agents/skills/react-compiler-rules/SKILL.md`). Suppressing the highlight by adding manual memo defeats the diagnostic.
...
steps:
  ...
  - name: Build ${{ env.APP }}
    ...
    run: bun run build --filter=@house-manager/${{ env.APP }}
...
3. **Offline deep-link** — DevTools → Network → Offline → load a deep route directly. Confirm it renders from cache.

If step 3 fails, the offline contract is broken — see `apps/web/tests/shell.offline.spec.ts` for the regression test that should have caught it locally.

---

## Architecture — the offline deep-link contract

The single most important flow: **hitting a deep URL while offline must boot the SPA from cache, hydrate from IDB, and render — with no server round-trip**. Lose this and the iPad-over-LAN scenario breaks.

> **Editing this diagram?** Mermaid's sequence-diagram parser treats `+`, `;`, and `<` followed by a letter (e.g. `<SomeCard ...>`) as syntax tokens inside message and note text — *not* literal characters. Avoid those, then validate with `bunx --bun @mermaid-js/mermaid-cli -i diagram.mmd -o out.svg` before pushing.

```mermaid
sequenceDiagram
    autonumber
    participant User as User (iPad, offline)
    ...
    Note over User,SW: First visit was online, so SW and shell are precached.

    User->>SW: navigate to a deep route
    SW-->>User: precached index.html via navigateFallback
    User->>Bundle: parse shell, evaluate JS
    ...
    Suspense->>IDB: openDB(house-manager, v2)
    IDB->>IDB: upgrade callback runs cumulative migrations
    IDB-->>Suspense: connection ready
    Suspense->>IDB: getAll(store), get(settings)
    IDB-->>Suspense: raw rows
    Suspense->>Suspense: Zod safeParse rows, set resolvedSnapshot
    Suspense-->>Router: hydration promise resolved
    Router->>Router: ParamsSchema.parse coerces route params
    Router->>Atom: read atomWithIDB for the record id
    Atom->>Atom: lazy resolveCurrent, snapshot miss returns fallback
    Atom-->>Router: record returned with fallback values
    Router-->>User: render the route from URL params and atom value

    Note over User,Channel: User triggers a state change, write-through path.

    User->>Atom: set next value
    Atom->>Atom: Schema.parse(next), Pillar 2 contract
    Atom->>Atom: update in-memory storage atom
    Atom->>Persist: call persist(value)
    Persist->>Persist: schedule with 150ms debounce, per-key
    Persist->>IDB: db.put(store, value)
    IDB-->>Persist: write committed
    Persist->>Channel: postMessage on house-manager:idb
    Channel-->>User: other tabs and Storybook iframe re-hydrate
```

Four load-bearing properties this diagram encodes:

- **No network.** Steps 1–3 use only the SW's precache. The router resolves the deep route client-side and never asks a server — for a route that is *not* in the prerender list, Workbox's `navigateFallback: "/index.html"` (with a denylist that excludes `/assets/*`, image/font extensions, and `/sw.js`) hands back the canonical shell and the router takes over from there.
...
- **Two-stage URL → state resolution.** Step 14 (`ParamsSchema.parse`) coerces URL segments to typed route params (e.g. via `z.coerce.number().int()`). Step 16 returns the atom's `fallback` because IDB is empty on a cold offline boot — the rendered route displays values from the URL plus fallback defaults from the atom, even though no record exists yet. Writing one (step 19) creates it.
- **Parse on every set, debounced write-through.** Step 20 is Pillar 2 in action — `Schema.parse(next)` inside `atomWithIDB`'s setter (`apps/web/app/lib/atom-with-idb.ts`). An invalid set throws and propagates to the root `errorComponent`. Steps 22–24 isolate IDB writes from React's render pipeline: `persist.ts` schedules a 150ms timer per key, coalescing rapid updates into a single `db.put`, then posts `{ store, key }` on the `house-manager:idb` BroadcastChannel so other tabs and the Storybook iframe re-hydrate. The value itself is *not* on the channel — the receiver re-reads from IDB, keeping IDB the single source of truth (Pillar 3).

This contract is verified end-to-end by `apps/web/tests/shell.offline.spec.ts` — currently `.skip`'d pending the Workbox / TanStack-Start `_shell.html` rename ordering fix (see [Known gaps](#known-gaps)).

---

## Scaffolding new apps

Add a new house-manager app — same toolchain, same Pillars, same gate — with the TurboRepo generator at `turbo/generators/config.ts`.
...
The generator templates substitute `{{name}}` everywhere it matters: `package.json`'s `@house-manager/<name>`, the IDB database name, the `BroadcastChannel` name, the PWA manifest, the `index.html` `<title>`, the `vite.config.ts` `base` path for project pages, and the default `VITE_GAME_TITLE` in `.env`.
...
├── biome.json                        # extends @house-manager/biome-config
├── stylelint.config.mjs              # extends @house-manager/stylelint-config
├── tsconfig.json
├── scripts/
...
- [ ] A new route declares its OWN `errorComponent` for tight recovery (root is the safety net, not the first line)?
...
**Biome can't resolve `@house-manager/biome-config/biome.json`.**
Run `bun install` so workspace symlinks land in `node_modules/@house-manager/`. If it still fails, swap to a relative extends in `biome.json`: `"extends": ["./packages/biome-config/biome.json"]`.
...
The Tailwind plugin (`@dreamsicle.io/stylelint-config-tailwindcss`) isn't installed or `stylelint.config.mjs` isn't extending `@house-manager/stylelint-config`. Re-run `bun install`.
...
- [`apps/web/tests/shell.offline.spec.ts`](./apps/web/tests/shell.offline.spec.ts) — the load-bearing offline-deep-link test the architecture diagram describes.
