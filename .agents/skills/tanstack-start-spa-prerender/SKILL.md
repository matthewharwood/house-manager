...
```
In house-manager every route effectively behaves this way — the shell is static, the client fills in real content from IDB. `ssr: false` makes that explicit per-route when needed.
