...
test("ProgressRow writes a record to IDB after Save click", async ({ page }) => {
  ...
  const stored = await page.evaluate(async () => {
    ...
    const name = db.find((d) => d.name === "house-manager")?.name;
    if (!name) return null;
