...
```jsonc
{
  ...
  "ignoreDependencies": [
    "@house-manager/biome-config",        // consumed via `extends` in biome.json
    "@house-manager/tsconfig",            // consumed via `extends` in tsconfig.json
...
### Dead-code gate vs health advisory — the house-manager partition

The default `fallow` invocation runs three things: dead-code, duplication, and complexity health. house-manager treats them differently:
...
house-manager treats the `circular-dependencies` rule as load-bearing. When two modules need to reach into each other, **extract the shared symbol(s) into a leaf module** and have BOTH original modules depend on the leaf. Never resolve a cycle with `// fallow-ignore-next-line circular-dependency`.
