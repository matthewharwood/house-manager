---
name: biome
description: "Biome 2.x as the formatter + linter for `.ts`, `.tsx`, `.js`, and `.json` in house-manager — `biome ci` is the gate, watch mode runs alongside `bun run dev`, and Biome MUST NOT touch CSS. Triggers on: biome, biome.json, biome ci, biome lint, biome format, biome watch, biome rule."
license: MIT
---
...
When extracting shared config to `packages/biome-config/biome.json` and extending from the workspace root via `"extends": ["@house-manager/biome-config/biome.json"]`, the **shared config must declare `"root": false`** at the top level:
...
### Minimal `biome.json` for house-manager
```json
{
  "$schema": "https://biomejs.dev/schemas/2.4.13/schema.json",
...
Biome 2.x ships **no native CLI watcher** — the `--watch` flag does not exist on `biome check`. house-manager wraps Biome in `chokidar-cli` (root `devDependencies`) so the CLI watcher is real and the IDE stays non-authoritative.
