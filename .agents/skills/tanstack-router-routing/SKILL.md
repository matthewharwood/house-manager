...
description: "TanStack Router file-based routing for house-manager — `createFileRoute`, route trees, route params, search params, `Link` / `useNavigate` / `useRouter`, and Zod-validated params via `validateSearch` (defers to zod for the schemas). Triggers on: tanstack router, file-based routing, createRoute, route param, search params, useNavigate, Link, route tree."
license: MIT
---
...
What this catches in house-manager specifically:
