---
name: kill-servers
description: Use when asked to kill stale house-manager dev, preview, or Storybook server processes and free ports before `bun run check`, Playwright, or the five-phase-pass skill. Recreates the old /kill-servers command as a Codex skill.
---

<objective>
Stop every local house-manager dev/preview/storybook server from prior runs and confirm their TCP ports are free. Idempotent and safe to run any time. Use as a pre-step before any gate or test run.
</objective>
...
```bash
# 1. Kill known house-manager server processes by command-line pattern.
...
<caveats>
- `pkill -f "storybook dev"` matches **any** storybook dev process on the machine, not just house-manager's. If you're running another Storybook project simultaneously, this will kill it too. For multi-project safety, use the port-targeted form: `lsof -ti :6006 | xargs kill`. Same caveat for `vite dev` / `vite preview`.
