---
name: tanstack
description: "Router skill for TanStack Start (`@tanstack/react-start`) in house-manager — dispatches to the SPA-prerender sub-skill, the routing sub-skill, or the PWA-deep-links sub-skill. SPA mode + full prerender targets GitHub Pages; no server functions, no server loaders. Triggers on: tanstack, tanstack start, tanstack router overview, which tanstack skill."
license: MIT
---

Entry point for any TanStack-shaped question in house-manager. The framework is TanStack Start running in **SPA mode with full prerender** for GitHub Pages — there is no runtime server in production. This skill exists to dispatch; sub-skills carry the rules.
