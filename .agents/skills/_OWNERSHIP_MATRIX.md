# Ownership Matrix — house-manager skill system

Authoritative contract for the house-manager skill collection. Every later prompt that builds a `SKILL.md` must read and obey this file. AGENTS.md owns the Four Pillars and stack pins; skills do not restate Pillar text — they reference Pillars by name only.
...
| symphony | narrow | — | OpenAI Symphony reference-runner integration: ignored local install under `tmp/symphony`, workflow/spec package `@house-manager/symphony-first-five`, Linear env wiring, generated-app-safe `bun run symphony` / `bun run dev:symphony`, and `.symphony/` runtime artifacts. | symphony, OpenAI Symphony, Linear orchestrator, dev:symphony, WORKFLOW.md, first-five backlog |
...
| `@theme` | tailwind |
| `.nvmrc` | node |
| `addChild` | (out of scope — pixijs collection, not house-manager) |
| `anime v4` | animejs |
| `anime utils` | animejs |
| `anime.js` | animejs |
| `animejs` | animejs |
...
(Note: the row for `addChild` is intentionally listed as out-of-scope to flag a potential collision with the existing `pixijs-scene-container` skill — that keyword stays with pixijs and is not claimed by any house-manager skill.)
...
- **house-manager rule:** runtime parsing is dev-only via `defineComponent(schema, fn)`; production tree-shakes the parse call.
...
- **house-manager rule:** the root `idbHydrationPromise` resolves once; `atomWithIDB` reads sync after that.
...
### jotai (latest 2.x)
- `atom`, `useAtom`, `useAtomValue`, `useSetAtom`, `useStore`, `Provider`, `atomWithStorage`, `loadable`, `unwrap`, `useHydrateAtoms`. (`atomFamily` is intentionally excluded — house-manager uses the `atomWithIDB` key + module-scope `Map<id, atom>` pattern; see `jotai/SKILL.md` parameterized-atoms section.)
...
### nitro (Nitro v3)
- `defineNitroConfig({ preset: "github_pages", baseURL: "/house-manager/", prerender: { routes: [...] } })`.
...
- **house-manager rule:** every type that has a Zod schema is `z.infer<typeof Schema>` — never hand-written.
...
- **house-manager rule:** ASK FIRST before writing any Playwright test. The user owns the structural decisions.
