import type { EntityRecord } from "./entity-schema";

// Starter content seeded by migrations into the default "Main House" namespace
// (ns-main). Extra recipe fields (structured ingredients, baseServings, …) ride
// alongside the EntityRecord base and are validated by RecipeSchema on read.
export const SEED_RECIPES: EntityRecord[] = [
  {
    id: "recipe-coconut-chia-oat-pudding",
    type: "recipe",
    namespaceId: "ns-main",
    createdAt: "2026-06-21T08:00:00.000Z",
    updatedAt: "2026-06-21T08:00:00.000Z",
    title: "Coconut Chia Oat Pudding in a Jar",
    mealType: "breakfast",
    cadence: "Biweekly",
    baseServings: 1,
    forWho: ["person-matthew", "person-daisy"],
    ingredients: [
      { name: "Chia seeds", amount: 4, unit: "tbsp", section: "", note: "" },
      { name: "Rolled oats", amount: 3, unit: "tbsp", section: "", note: "" },
      { name: "Coconut milk", amount: 1, unit: "cup", section: "", note: "" },
      { name: "Regular milk", amount: 0.25, unit: "cup", section: "", note: "plus more if needed" },
      { name: "Honey", amount: 1, unit: "tbsp", section: "", note: "to taste" },
      { name: "Cinnamon", amount: 0.25, unit: "tsp", section: "", note: "¼–½" },
      { name: "Vanilla extract", amount: 0.5, unit: "tsp", section: "", note: "" },
      { name: "Salt", amount: 0, unit: "", section: "", note: "tiny pinch" },
      {
        name: "Ground flaxseed",
        amount: 1,
        unit: "tbsp",
        section: "Toppers",
        note: "≈7 g · ground, not whole — whole passes through undigested",
      },
      { name: "Hemp hearts", amount: 1, unit: "tbsp", section: "Toppers", note: "≈10 g" },
      { name: "Goji berries", amount: 1, unit: "tbsp", section: "Toppers", note: "≈6 g · dried" },
      {
        name: "Shredded dark chocolate",
        amount: 1,
        unit: "tbsp",
        section: "Toppers",
        note: "≈6 g · Montezuma Black 100% — intense, start at 1 tsp",
      },
      {
        name: "Granola",
        amount: 1,
        unit: "tbsp",
        section: "Toppers",
        note: "≈7 g · just a little, for crunch",
      },
    ],
    method: [
      "1. In your jar, add coconut milk, regular milk, honey, cinnamon, vanilla, and salt.",
      "2. Stir or shake until the honey is mixed in.",
      "3. Add chia seeds and rolled oats.",
      "4. Stir really well.",
      "5. Wait 5–10 minutes, then stir again so the chia doesn't clump.",
      "6. Refrigerate at least 2 hours, ideally overnight.",
      "7. Before eating, stir again. If it's too thick, add a splash of regular milk.",
      "8. Top with the ground flaxseed, hemp hearts, goji berries, and shredded dark chocolate. Add the granola last, right before eating, so it stays crunchy.",
    ].join("\n"),
    notes: [
      "Kids won't eat it — this one's for Matthew & Daisy.",
      "",
      "Optional add-ins —",
      "Best combo: ½ banana (sliced or mashed), 1–2 tbsp chopped walnuts or pecans, 1 tbsp shredded coconut.",
      "Tropical: mango, shredded coconut, a little lime zest.",
      "",
      "Texture: with 4 tbsp chia + oats it gets pretty thick. For creamier the next day, add another 2–4 tbsp milk and stir.",
    ].join("\n"),
  } as EntityRecord,
  {
    id: "recipe-kimchi-dill-egg-bowl",
    type: "recipe",
    namespaceId: "ns-main",
    createdAt: "2026-06-21T08:05:00.000Z",
    updatedAt: "2026-06-21T08:05:00.000Z",
    title: "Kimchi Dill Egg Breakfast Bowl",
    mealType: "breakfast",
    cadence: "",
    baseServings: 1,
    forWho: ["person-daisy"],
    ingredients: [
      {
        name: "Cooked wild rice / purple mixed rice",
        amount: 100,
        unit: "g",
        section: "Rice + greens",
        note: "",
      },
      { name: "Raw spinach", amount: 30, unit: "g", section: "Rice + greens", note: "" },
      { name: "Hard-boiled egg, peeled", amount: 50, unit: "g", section: "Protein", note: "" },
      { name: "Nonfat Greek yogurt", amount: 170, unit: "g", section: "Protein", note: "" },
      { name: "Tomato", amount: 80, unit: "g", section: "Fresh crunch", note: "" },
      { name: "Cucumber with peel", amount: 80, unit: "g", section: "Fresh crunch", note: "" },
      { name: "Kimchi", amount: 18, unit: "g", section: "Fresh crunch", note: "" },
      { name: "Avocado flesh", amount: 35, unit: "g", section: "Fat + texture", note: "" },
      { name: "Olive oil", amount: 5, unit: "g", section: "Fat + texture", note: "" },
      { name: "Ground flaxseed", amount: 8, unit: "g", section: "Fat + texture", note: "" },
      { name: "Lemon juice", amount: 7, unit: "g", section: "Seasoning", note: "" },
      { name: "Fresh dill", amount: 2, unit: "g", section: "Seasoning", note: "" },
      { name: "Garlic powder", amount: 0.3, unit: "g", section: "Seasoning", note: "" },
      { name: "Smoked paprika", amount: 0.5, unit: "g", section: "Seasoning", note: "" },
      { name: "Black pepper", amount: 0.2, unit: "g", section: "Seasoning", note: "" },
      { name: "Himalayan salt", amount: 0.5, unit: "g", section: "Seasoning", note: "" },
    ],
    method: [
      "1. Warm 100g cooked rice.",
      "2. Wilt 30g spinach, then mix it into the rice.",
      "3. Mix the sauce: 170g yogurt, 8g flaxseed, 7g lemon juice, 2g dill, 0.3g garlic powder, 0.5g smoked paprika, 0.2g black pepper, and 0.5g salt.",
      "4. Dice 80g tomato, 80g cucumber, 35g avocado, and 18g kimchi.",
      "5. Add rice/spinach to the bowl.",
      "6. Add tomato, cucumber, avocado, kimchi, and 50g hard-boiled egg.",
      "7. Spoon the yogurt sauce over top.",
      "8. Finish with 5g olive oil.",
    ].join("\n"),
    notes: [
      "Base recipe is one serving — use the Days counter (1–7) to batch a few days at once.",
    ].join("\n"),
  } as EntityRecord,
];

// The household — first-class people referenced by id from recipes/chores
// (see PersonSchema). Daisy is the wife/mom, Matthew the dad, Dean & Ava the kids.
export const SEED_PEOPLE: EntityRecord[] = [
  {
    id: "person-daisy",
    type: "person",
    namespaceId: "ns-main",
    createdAt: "2026-06-21T08:00:00.000Z",
    updatedAt: "2026-06-21T08:00:00.000Z",
    name: "Daisy",
    role: "Mom",
    aliases: [],
    birthday: "1984-02-27",
    tags: ["family"],
    archived: false,
  } as EntityRecord,
  {
    id: "person-matthew",
    type: "person",
    namespaceId: "ns-main",
    createdAt: "2026-06-21T08:00:00.000Z",
    updatedAt: "2026-06-21T08:00:00.000Z",
    name: "Matthew",
    role: "Dad",
    aliases: [],
    birthday: "1985-09-03",
    tags: ["family"],
    archived: false,
  } as EntityRecord,
  {
    id: "person-dean",
    type: "person",
    namespaceId: "ns-main",
    createdAt: "2026-06-21T08:00:00.000Z",
    updatedAt: "2026-06-21T08:00:00.000Z",
    name: "Dean",
    role: "Son",
    aliases: [],
    birthday: "2018-02-28",
    tags: ["family"],
    archived: false,
  } as EntityRecord,
  {
    id: "person-ava",
    type: "person",
    namespaceId: "ns-main",
    createdAt: "2026-06-21T08:00:00.000Z",
    updatedAt: "2026-06-21T08:00:00.000Z",
    name: "Ava",
    role: "Daughter",
    aliases: [],
    birthday: "2024-12-12",
    tags: ["family"],
    archived: false,
  } as EntityRecord,
];

// The open household-help req, seeded into the ATS (§8). One posting per
// namespace; candidates reference it implicitly by living in the same namespace.
export const SEED_POSTINGS: EntityRecord[] = [
  {
    id: "posting-household-ops-manager",
    type: "jobPosting",
    namespaceId: "ns-main",
    createdAt: "2026-06-22T08:00:00.000Z",
    updatedAt: "2026-06-22T08:00:00.000Z",
    title: "Household & Family Operations Manager",
    location: "Portland, OR — Cedar Mill / 97229 (in-person)",
    schedule: "Full-time · bookended (morning push, evening wind-down)",
    pay: "$25/hour · contract-to-hire (W-2)",
    status: "open",
    summary: [
      "We're a busy Portland-area family looking for an exceptional, proactive person to help run our household and keep daily life moving smoothly. This is a true generalist role — part household manager, part family assistant, part operations right hand. If you love bringing order to chaos, anticipating what's needed before you're asked, and owning the details so a family doesn't have to think about them, this is for you.",
      "",
      'The rhythm is full-time and "bookended": in and out across the day, with a natural morning push and evening wind-down. Some things (like dishes) just need to get done sometime that day — we trust you to find the cadence that works.',
    ].join("\n"),
    responsibilities: [
      "Home & meals",
      "• Keep common spaces clean, tidy, and reset daily",
      "• Run the dishes through the day (your call on timing)",
      "• Plan and prep simple, healthy family meals; manage groceries and the pantry",
      "• Light yard work and outdoor upkeep",
      "• Laundry and general home organization",
      "",
      "Kids",
      "• Babysit and supervise our young children with warmth and attentiveness",
      "• Drive the kids to school, activities, and appointments",
      "",
      "Pets",
      "• Daily dog walks and basic pet care",
      "",
      "Errands & logistics",
      "• Groceries, returns, pickups, and general errands",
      "• Manage trash and recycling — including the annoying ones like e-waste and electronics",
      "• Proactively solve the little problems that pop up, and look for ways to make our days run better",
      "",
      "Operations & admin",
      "• Book appointments and meetings; help keep the family calendar straight",
      "• Handle phone calls and read/triage email",
      "• Use our custom daily triage app to plan and track what's getting done",
      "• Light office-manager support for the family's side businesses as needed",
    ].join("\n"),
    requirements: [
      "Who you are",
      "• Organized, dependable, and genuinely proactive — you anticipate needs and don't wait to be told",
      "• Calm under shifting priorities and good at juggling many small things",
      "• Warm and trustworthy with kids and adults; discreet about family matters",
      "• A natural problem-solver who likes optimizing systems",
      "• Comfortable with apps, phones, and basic tech (phone and/or laptop provided if needed)",
      "",
      "Logistics",
      "• $25/hour, full-time, contract-to-hire — a strong fit can convert to a permanent W-2 role",
      "• Based in Portland / Cedar Mill (97229); this is an in-person role",
      "• Valid driver's license and reliable vehicle required (mileage reimbursed)",
      "• Offer contingent on background check and references",
    ].join("\n"),
  } as EntityRecord,
];
