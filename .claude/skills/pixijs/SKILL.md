...
## house-manager integration (read before mounting Pixi inside a component)

PixiJS is **first-party** in house-manager at `pixi.js@^8.18.1` for any canvas-based UI. It is a **side channel** the same way anime.js is — render functions stay pure; everything that touches Pixi (`new Application()`, `Ticker.add(...)`, sprite mutation, scene-graph reads) lives inside `useEffect` / `useLayoutEffect` / event handlers, never during render.
