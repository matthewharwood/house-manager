...
description: Bun 1.3.13 package manager for the house-manager monorepo — `bun install`, `bun add`, the text lockfile `bun.lock`, workspace globs, the `packageManager` pin, and overrides. Triggers on: bun install, bun add, bun remove, bun.lockb, bun workspace, bun overrides, bun lockfile.
license: MIT
---

Sub-skill of `bun`. Owns dependency installation, lockfile, and workspace topology for the house-manager TurboRepo monorepo.
...
{
  "dependencies": {
    "@house-manager/schemas": "workspace:*"
  }
}
```
