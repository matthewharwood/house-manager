...
- **`Flashcard`** — `{ id, title, content }`. `content` is a discriminated union on `type`:
  ...
  - `card-mini-game` — `{ prompt, pairs? }` — a drag-and-drop practice node that **must** be built on `@house-manager/cards` (CLAUDE.md rule). Use sparingly.
- **`Curriculum`** — `{ id, title, source, nodes: Flashcard[], edges: Edge[] }`. `Edge` = `{ from, to }`, a directed prerequisite.

Do **not** emit scheduling state. Spaced-repetition `CardState` is runtime progress (IDB, keyed by flashcard id via `getCurriculumProgressAtom`) — content and progress are separate, exactly like the static `Card`/`Deck` vs. progress split in `@house-manager/srs`.
...
3. **Map the graph** — dedupe into nodes with stable kebab-case `id`s; draw `edges` foundational→advanced. Keep it a DAG; `rootIds` (from `@house-manager/curriculum`'s `graph.ts`) are the entry points; `layeredTree`/`forceLayout` will lay it out for the PixiJS views.
...
- Reuse `@house-manager/curriculum` helpers (`buildCurriculumGraph`, `rootIds`, `prerequisites`, `dueFlashcardIds`) rather than re-deriving graph logic.
...
- Don't hand-roll drag-and-drop for `card-mini-game`; compose `@house-manager/cards`.
