...
This is a cross-cutting authoring policy, not a tech. Anything that touches an owned house-manager tech (state, storage, schemas, animation, canvas, routing, motion) defers to that tech's skill.
...
- **Date arithmetic** — no date library is pinned in house-manager. Surface a stack-pin decision with the user before adding `date-fns` or anything else; `Intl.DateTimeFormat` covers display-only needs.
...
- **Don't import `bcrypt` / `argon2`** — there is no server in house-manager; auth doesn't apply on GH Pages.
