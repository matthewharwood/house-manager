---
name: stylelint
description: Stylelint with the Tailwind v4 plugin for every `.css` file in house-manager — CSS lint exclusively, `--max-warnings 0` is the gate, and the CLI watcher is the source of truth (not the IDE). Triggers on: stylelint, stylelintrc, stylelint tailwind, stylelint css, stylelint watch, stylelint plugin, max-warnings.
license: MIT
---
...
### Workspace script
Stylelint 16 dropped the native `--watch` flag. house-manager wraps Stylelint in `chokidar-cli` (root `devDependencies`) so the CLI watcher is real and the IDE plugin stays non-authoritative.
