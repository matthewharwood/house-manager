---
name: bun-runtime
description: Bun 1.3.13 runtime APIs for tooling and scripts in house-manager — Bun.serve, Bun.file, Bun.$, bunx — with the rule that browser code is bundled by Vite, never executed directly by Bun. Triggers on: bun runtime, Bun.serve, Bun.file, Bun.$, bun shell, bunx, bun script.
license: MIT
---
...
```
Use only for Playwright fixtures or local dev tooling. The house-manager app itself ships as static files — see `nitro`.
