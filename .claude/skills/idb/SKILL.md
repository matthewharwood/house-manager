...
- **`if (oldVersion < N)` cascading checks** (Biome-clean, what house-manager uses today). Each `if` runs for every user below that version, achieving the same cumulative effect.
...
const channel = new BroadcastChannel("house-manager:idb");
