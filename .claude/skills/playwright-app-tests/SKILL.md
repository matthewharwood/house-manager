...
description: Playwright end-to-end tests pointed at the running house-manager app — route workflows under `apps/web/tests/app/`, real IDB hydration via `page.evaluate`, BroadcastChannel cross-tab verification, IDB-seeded fixtures (no mocks). Triggers on: playwright app test, e2e workflow, app-level test, route workflow test, idb seeded test, idb assertion.
license: MIT
---
...
const test = base.extend<{ seededPage: typeof base extends never ? never : Awaited<ReturnType<typeof base["page"]["fixture"]>> }>({
  ...
  seededPage: async ({ page }, runFixture) => {
    ...
    await page.addInitScript(() => {
      const open = indexedDB.open("house-manager", 2);
...
```
`context.newPage()` shares a `BrowserContext` (and therefore the `BroadcastChannel`) across pages. See `idb` for the channel name (`house-manager:idb`) and the message shape.
