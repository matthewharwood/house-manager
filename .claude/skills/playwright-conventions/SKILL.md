...
description: The shared conventions every Playwright spec in house-manager obeys — the load-bearing ASK-FIRST rule (Pillar 4), selector strategy (`getByRole` > `getByTestId` > `getByText`), fixture patterns (fresh IDB, seeded IDB, offline, throttled), reduced-motion forced on, and the `playwright.config.ts` shape. Triggers on: playwright fixture, playwright selector, role selector, getByRole, test-id, fresh idb fixture, reduced motion playwright, ask first test.
license: MIT
---

Sub-skill of `playwright`. The **canonical home of the ASK-FIRST rule** plus every cross-cutting Playwright convention in house-manager: selector preference order, fixture patterns (fresh IDB / seeded IDB / offline / throttled), reduced-motion forcing, the `playwright.config.ts` shape, and the unit-vs-E2E partition. The four other Playwright sub-skills defer here for these contracts.
...
// XPath — only for legacy DOM you can't change. Almost never needed in house-manager.
```
...
expect(await page.title()).toBe("house-manager");
