---
description: Extract a typed @house-manager/curriculum from a GitHub repo or PDF — read the source, abstract its knowledge into a graph of flashcard lesson nodes with mapped prerequisite edges, emit CurriculumSchema-validated data into apps/web/app/data/, and leave the gate green.
argument-hint: <github-repo-url | path/to.pdf> [curriculum-id]
---

<objective>
Turn a single source of knowledge (one GitHub repo or one PDF) into ONE homogeneously-typed `Curriculum` for the learning game: a graph of `Flashcard` lesson nodes with mapped prerequisite `edges`, coupled to its `Source`. The output is committed TypeScript data validated by `@house-manager/curriculum`'s Zod schema — no backend, no runtime extraction. Authoring-time only.
</objective>
...
<contract>
The output MUST satisfy `CurriculumSchema` from `@house-manager/curriculum` (`packages/curriculum/src/schema.ts`). Read that file first — it is the source of truth. Shape:

- `Source` — `{ kind: "github-repo", url, ref? }` or `{ kind: "pdf", href, pages? }`.
- `Flashcard` — `{ id, title, content }` where `content` is the discriminated union: `read` (markdown), `multiple-choice` (question/options/answerIndex), `video` (src/caption), `card-mini-game` (prompt/pairs). Prefer `read` and `multiple-choice` (the fully-built renderers); use `card-mini-game` only for drag-and-drop practice (it composes `@house-manager/cards`).
...
- Card drag-and-drop nodes (`card-mini-game`) MUST use `@house-manager/cards` (CLAUDE.md rule); don't hand-roll.
