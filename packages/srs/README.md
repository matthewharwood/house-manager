...
A small, bespoke spaced-repetition engine for the house-manager stack. A **deck**
...
```ts
import { review, createDeck, reviewCard } from "@house-manager/srs";        // pure core (zod only)
...
```ts
import { createDeck, getDueCards, reviewCard, exportDeck, importDeck } from "@house-manager/srs";
...
const store = createDeckStore({
  database: "@house-manager/web/srs", // ⚠ origin-unique — namespace by repo + app
  content: Flashcard,
});
